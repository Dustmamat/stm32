...
int main(void)
{
  ...
  MX_TIM3_Init();
  /* USER CODE BEGIN 2 */
  unsigned int half_period = 100;
  SysTick_Config(SystemCoreClock / 1000);
  LL_TIM_WriteReg(TIM3,CR1,LL_TIM_ReadReg(TIM3,CR1)|0x01);
  LL_TIM_WriteReg(TIM3,CCER,LL_TIM_ReadReg(TIM3,CCER)|0x01);
  LL_TIM_WriteReg(TIM3,CCR1,half_period);
  //Configure the ADC
  //unsigned int adc_enable_mask = 1<<31|1<<29|1<<30|0x01|0x02|1<<10; //SWSTART and ADON set to 1
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if((LL_TIM_ReadReg(TIM3,SR) &0x02) !=0){
		  ...
		  LL_TIM_WriteReg(TIM3,CCR1,LL_TIM_ReadReg(TIM3,CCR1)+half_period);
	  }


	  if((LL_ADC_ReadReg(ADC1,SR)&0x02)!=0){
		  LL_ADC_WriteReg(ADC1,SR,LL_ADC_ReadReg(ADC1,SR)&(~0x02));
		  half_period = LL_ADC_ReadReg(ADC1,DR)*1.08+933;
		  LL_ADC_WriteReg(ADC1,CR2,LL_ADC_ReadReg(ADC1,CR2)|1<<30);
